...
class Smooth(object):
    ...
    def __init__(self, base_classifier: torch.nn.Module, num_classes: int, sigma: float, confs: float, certify_mode: str):
        """
    ...
    def _sample_noise(self, x: torch.tensor, num: int, batch_size) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            for _ in range(ceil(num / batch_size)):
                ...
                if self.certify_mode == 'WithDetect':
                   logits = self.base_classifier(batch + noise)
                   predictions = logits.argmax(1)
                   prob_vector = F.softmax(logits, dim=1)
                   confs = torch.max(prob_vector,dim=1).values
                   predictions = predictions[confs > self.confs]#
