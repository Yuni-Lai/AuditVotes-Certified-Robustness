...
class GCNJaccard_Aug(nn.Module):
    ...
    '''
    def __init__(self, n_features, n_classes, n_hidden, edge_proportion, Ts, p_dropout=0.5,threshold1=0.0, threshold2=0.5, binary_feature=True,device='cuda'):
        super().__init__()
        self.conv1 = SparseGCNConv(n_features, n_hidden)
        self.conv2 = GCNConv(n_hidden, n_classes)
        self.p_dropout = p_dropout
    ...
    def set_jaccard_mask(self):
        self.J_mask, self.JAug_mask = self.get_Jaccard_mask(self.threshold1, self.threshold2)
        #testing
        self.J_mask_sparse = self.J_mask.to_sparse()
        self.JAug_mask_sparse = self.JAug_mask.to_sparse()
        self.JAug_mask_idx_all = self.JAug_mask_sparse.coalesce().indices()
        self.J_mask_idx_all = self.J_mask_sparse.coalesce().indices()

        #train
        subgraph_idx=torch.concatenate([self.idx['train'], self.idx['unlabeled']])
        self.n0_train = len(subgraph_idx)
        mask_temp = self.J_mask[subgraph_idx, :][:, subgraph_idx]
        self.J_mask_idx_train = mask_temp.to_sparse().coalesce().indices()
        mask_temp = self.JAug_mask[subgraph_idx,:][:,subgraph_idx]
    ...
    def batch_J_sparse(self,mask_idx,n,batch_size=1):
        ...
        #----------------------------------------------------
        # # Another Version (my implement) avoid OOM, similar speed:
        batch_mask_idx = []
    ...
    def Jaccard_mask_edge_idx(self, edge_idx, n):
    ...
    def get_threshold(self, adj_logits, adj_orig, n, batch_size):
        ...
        ADD = batch_size * int(E_prime * (1-(1/self.Ts)))  # number of edge to add
        DEL = 0 # batch_size * int((N_prime ** 2 - E_prime) * self.p_plus)  # number of edge to delete
    ...
    def forward(self, attr_idx, edge_idx, n, d): #default forward
        adj_sampled = self.Jaccard_mask_edge_idx(edge_idx, n)
...
class VAE(nn.Module):
    ...
    def test_all(self,edge_idx,attr_idx, n, d, test_idx):
        with torch.no_grad():
            ...
            adj_orig = adj_orig.clip(max=1.0)
...
class FAugGCN(nn.Module):
    def __init__(self, n_features, n_classes, n_hidden, dim_z,edge_proportion,Ts, p_dropout=0.5):
        super().__init__()
        self.nc_net = GCN(n_features, n_classes, n_hidden, p_dropout)  # node classification network
        self.ep_net = VAE(n_features, n_hidden, dim_z, ae=True) # edge prediction network
        self.threshold1 = 0.1
        self.threshold2 = 0.5
    ...
    def set_mask(self):
        self.S_mask1, self.S_mask2 = self.get_mask(self.threshold1, self.threshold2)
        #testing
        self.S_mask1_sparse = self.S_mask1.to_sparse()
        self.S_mask2_sparse = self.S_mask2.to_sparse()
        ...
        self.n0_train = len(subgraph_idx)
        mask_temp = self.S_mask1[subgraph_idx, :][:, subgraph_idx]
        self.S_mask1_idx_train = mask_temp.to_sparse().coalesce().indices()
        mask_temp = self.S_mask2[subgraph_idx,:][:,subgraph_idx]
...
class SAugGCN(nn.Module):
    def __init__(self, n_features, n_classes, n_hidden, dim_z, num_wegihts,edge_proportion,Ts,p_dropout=0.5):
        super().__init__()
        self.nc_net = GCN(n_features, n_classes, n_hidden, p_dropout)  # node classification network
        self.ep_net = SimGraph(n_features, num_wegihts) # edge prediction network
        self.threshold1 = 0.1
        self.threshold2 = 0.5
        self.edge_proportion=edge_proportion #(A-I).sum()/(N**2)
        self.Ts=Ts
...
class VGAE(nn.Module):
    """ GAE/VGAE as edge augmentation model """

    def __init__(self, dim_feats, dim_h, dim_z, gae=False):
        super(VGAE, self).__init__()
        self.gae = gae  # GAE or VGAE
        self.gcn_base = SparseGCNConv(dim_feats, dim_h)
        self.gcn_mean = GCNConv(dim_h, dim_z)
        self.gcn_logstd = GCNConv(dim_h, dim_z)
        self.EPS = 1e-15
        self.pos_weight = torch.FloatTensor([500])  # ratio of edge/none-edge
    ...
    def encoder(self, attr_idx, edge_idx, n, d):
        # GCN encoder
        hidden = F.relu(self.gcn_base(attr_idx, edge_idx, n, d))
        self.mean = F.relu(self.gcn_mean(hidden, edge_idx))
        if self.gae:
            # GAE (no sampling at bottleneck)
            Z = self.mean
        else:
            # VGAE
            self.logstd = F.relu(self.gcn_logstd(hidden, edge_idx))
            gaussian_noise = torch.randn_like(self.mean)
            sampled_Z = gaussian_noise * torch.exp(self.logstd) + self.mean
            Z = sampled_Z
        return Z
    ...
    def test_all(self, edge_idx, attr_idx, n, d, test_idx):
        with torch.no_grad():
            ...
            acc = accuracy_score(adj_orig.view(-1).cpu().numpy(), adj_recons.view(-1).cpu().numpy())
            auc = roc_auc_score(adj_orig.view(-1).cpu().numpy(), adj_logits.view(-1).cpu().numpy())
            print(acc, auc, best_threshold)
        return acc, auc, best_threshold

class GAugGCN(nn.Module):
    def __init__(self, n_features, n_classes, n_hidden, dim_z, p_dropout=0.5):
        super().__init__()
        self.nc_net = GCN(n_features, n_classes, n_hidden, p_dropout)  # node classification network
        self.ep_net = VGAE(n_features, n_hidden, dim_z, gae=True)  # edge prediction network
        self.temperature = 1.0
        self.alpha = 0.0001

    def sample_adj(self, adj_logits, adj_orig):
    ...
    def aug_adj(self, adj_logits, adj_orig, threshold1=0.05, threshold2=0.3):
        keep_adj = adj_logits > threshold1
        aug_adj = adj_orig * keep_adj
        add_adj = adj_logits > threshold2
        aug_adj = aug_adj + add_adj
        aug_adj = aug_adj.clamp(0, 1)
        # print("add_adj == True:", add_adj.sum()/(add_adj.shape[0]**2), "%")
        return aug_adj
    ...
    def forward(self, attr_idx, edge_idx, n, d):  # default forward
        adj_logits = self.ep_net(attr_idx, edge_idx, n, d)
        adj_logits = torch.sigmoid(adj_logits)
        adj_orig = torch.sparse.FloatTensor(edge_idx, torch.ones(edge_idx.size(1), device=edge_idx.device),
                                            torch.Size([n, n])).to_dense()
        adj_sampled = self.aug_adj(adj_logits, adj_orig, threshold1=0.0, threshold2=0.99)
        adj_sampled = adj_sampled.to_sparse().indices()
        hidden = self.nc_net(attr_idx, adj_sampled, n, d)
        return hidden
