...
def filter_data_for_idx(subgraph_attrs, subgraph_edges, n, d, device, labels, idx):
    '''filters attr, adj and labels for idx; also returns mapping from idx to corresponding indices in new objects'''
    subgraph_edges_subset = []
    subgraph_attrs_subset = []
    for subgraph_attr, subgraph_edge in zip(subgraph_attrs, subgraph_edges):
        adj = torch.sparse.FloatTensor(subgraph_edge, torch.ones(subgraph_edge.size(1), device=subgraph_edge.device),
                                            torch.Size([n, n])).to_dense()
        attr = torch.sparse.FloatTensor(subgraph_attr, torch.ones(subgraph_attr.size(1), device=subgraph_attr.device),torch.Size([n, d])).to_dense()

        adj_filtered = adj[idx,:]
        adj_filtered = adj_filtered[:,idx]
        # map attr
        attr_filtered = attr[idx]

        attr_idx = attr_filtered.to_sparse().indices()
        edge_idx = adj_filtered.to_sparse().indices()
        subgraph_edges_subset.append(edge_idx)
        subgraph_attrs_subset.append(attr_idx)
    ...
    return subgraph_attrs_subset, subgraph_edges_subset, labels_subset, mapping_proj
