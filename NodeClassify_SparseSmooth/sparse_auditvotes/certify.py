...
def certify(pre_votes,votes,labels,idx,args):
    ...
    if args.augmenter!='':
        df['certify_mode']=f'AuditV({args.augmenter})'
    else:
        df['certify_mode']=f'SparseSmooth'
    if args.filter !='':
        if len(args.filter.split('_'))>1:
            filter_name=args.filter.split('_')[0]
            df['certify_mode'] = f'AuditV({args.augmenter}+{filter_name})'
        else:
            df['certify_mode'] = f'AuditV({args.augmenter}+{args.filter})'
