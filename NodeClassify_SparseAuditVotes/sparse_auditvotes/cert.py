...
def binary_certificate_grid(pf_plus, pf_minus, p_emps, reverse=False, regions=None, max_ra=None, max_rd=None, progress_bar=True):
    ...
    radii = np.zeros((n_nodes, max_ra + 1, max_rd + 1))
...
def binary_certificate(votes, pre_votes, conf_alpha, pf_plus, pf_minus,type):
    """
    ...
    p_emps = p_lower_from_votes(
        votes=votes, pre_votes=pre_votes, alpha=conf_alpha, n_samples=votes.sum(1))
    if type=="r_d":
        max_ra = 0
        max_rd = 40
    elif type=="r_a":
    ...
    grid_base, regions, base_max_ra, base_max_rd = binary_certificate_grid(pf_plus=pf_plus, pf_minus=pf_minus,
                                                                                   p_emps=p_emps, reverse=False, progress_bar=True,max_ra=max_ra, max_rd=max_rd)

    # multi-class certificate ***
    p_lower, p_upper = p_lower_upper_from_votes(
        votes=votes, pre_votes=pre_votes, conf_alpha=conf_alpha, n_samples=votes.sum(1))
